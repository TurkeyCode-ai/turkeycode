...
`;
  ...
  private async preflightCheck(): Promise<void> {
    ...
    // Use the model the BUILD will use. With no model, Claude Code falls back
    // to the user's default — and if that tier is out of credit, preflight
    // dies before a single phase runs, reporting "make sure you have an
    // active subscription" while three other tiers answer fine. That is
    // exactly what happened: TURKEYCODE_MODEL=sonnet was set, the build would
    // have worked, and preflight failed on the default anyway.
    const preflightModel = getModelForPhase('build');
    ...
    if (testResult.exitCode !== 0 || testResult.stdout.trim().length < 3) {
      ...
      console.error(`\nModel used for preflight: ${preflightModel || '(Claude Code default)'}`);
      console.error('\nPossible fixes:');
      console.error('1. Run "claude" once to complete initial setup');
      console.error('2. If that model is out of credit, switch it:');
      console.error('     TURKEYCODE_MODEL=sonnet turkeycode resume');
      console.error(`3. Try: claude --print --model ${preflightModel || 'sonnet'} "hello"\n`);
      process.exit(1);
    }
